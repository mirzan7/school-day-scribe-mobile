...
const PrincipalReportTab = ({ user, getActivitiesByDate, getActiveDate, approveActivity, rejectActivity }) => {
  ...
  const allTeachers = useMemo(() => {
    const teachers = selectedActivities.reduce((acc, activity) => {
      if (!acc.find(t => t.id === activity.teacherId)) {
        acc.push({
          id: activity.teacherId,
          name: activity.teacherName,
          department: 'Science' // Placeholder, you might want to fetch this from a user context/service
        });
      }
      return acc;
    }, []);
